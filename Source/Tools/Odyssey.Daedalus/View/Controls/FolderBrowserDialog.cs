...
public class FolderBrowserDialog : IDisposable //, IDialogControlHost
{
    ...
    public FolderBrowserDialog(string title)
    {
        fileNames = new Collection<string>();
        this.title = title;
    }
    ...
    public bool? ShowDialog()
    {
        ...
        {
            MessageBox.Show("Folder browser dialog unavailable.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        finally
        {
    ...
    {
        private readonly FolderBrowserDialog parent;
        private bool firstFolderChanged = true;

        public NativeDialogEventSink(FolderBrowserDialog commonDialog)
        {
