...
namespace Odyssey.Graphics
{
    ...
    public sealed class Image : Component
    {
        ...
        private static readonly List<LoadSaveDelegate> loadSaveDelegates = new List<LoadSaveDelegate>();
        ...
        /// <param name="pitchFlags">The <see cref="PitchFlags"/></param>
        ...
        protected override void Dispose(bool disposeManagedResources)
        {
            ...
            {
                SharpDX.Utilities.FreeMemory(buffer);
            }
        ...
        /// </summary>
        /// <param name="dataBuffer">Pointer to an unmanaged memory. If <see cref="makeACopy"/> is false, this buffer must be allocated with <see cref="SharpDX.Utilities.AllocateMemory"/>.</param>
        ...
        /// Loads an image from an unmanaged memory pointer.
        /// </summary>
        /// <param name="dataPointer">Pointer to an unmanaged memory. If <see cref="makeACopy"/> is false, this buffer must be allocated with <see cref=""SharpDX.Utilities.AllocateMemory"/>.</param>
        ...
        public static Image Load(Stream imageStream)
        {
            ...
            if (nativeImageStream != null)
            {
                ...
                try
                {
                    ...
                    imageBuffer = SharpDX.Utilities.AllocateMemory(imageSize);
                ...
                finally
                {
                    if (image == null)
                    {
                        SharpDX.Utilities.FreeMemory(imageBuffer);
                    }
                }
                return image;
            }
            ...
            return Load(SharpDX.Utilities.ReadStream(imageStream));
        }
        ...
        public static Image Load(string fileName)
        {
            ...
            try
            {
                ...
                memoryPtr = SharpDX.Utilities.AllocateMemory(size);
                stream.Read(memoryPtr, 0, size);
            }
            catch (Exception)
            {
                if (memoryPtr != IntPtr.Zero)
                    SharpDX.Utilities.FreeMemory(memoryPtr);
                throw;
            }
            finally
            {
        ...
        {
            return (from loadSaveDelegate in loadSaveDelegates
                where loadSaveDelegate.Load != null
                select loadSaveDelegate.Load(dataPointer, dataSize, makeACopy, handle)).FirstOrDefault(
                    image => image != null);
        }
        ...
        internal unsafe void Initialize(ImageDescription description, IntPtr dataPointer, int offset, GCHandle? handle, bool bufferIsDisposable, PitchFlags pitchFlags = PitchFlags.None)
        {
            ...
            {
                buffer = SharpDX.Utilities.AllocateMemory(totalSizeInBytes);
                offset = 0;
        ...
        internal static Image LoadTKTX(IntPtr dataPointer, int dataSize, bool makeACopy, GCHandle? handle)
        {
            ...
            if (makeACopy)
            {
                var temp = SharpDX.Utilities.AllocateMemory(dataSize);
                SharpDX.Utilities.CopyMemory(temp, dataPointer, dataSize);
                dataPointer = temp;
            }
