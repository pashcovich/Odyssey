...
namespace Odyssey.Graphics
{
    ...
    new LegacyMap( Format.B4G4R4A4_UNorm,     ConversionFlags.Expand
        ...
        private static unsafe bool DecodeDDSHeader(IntPtr headerPtr, int size, DDSFlags flags, out ImageDescription description, out ConversionFlags convFlags)
        {
            ...
            if (size < (SharpDX.Utilities.SizeOf<DDS.Header>() + sizeof(uint)))
                return false;
            ...
            if (header.Size != SharpDX.Utilities.SizeOf<DDS.Header>() || header.PixelFormat.Size != SharpDX.Utilities.SizeOf<DDS.PixelFormat>())
                return false;
            ...
            if ((header.PixelFormat.Flags & DDS.PixelFormatFlags.FourCC) != 0 && (new FourCC('D', 'X', '1', '0') == header.PixelFormat.FourCC))
            {
                ...
                if (size < (SharpDX.Utilities.SizeOf<DDS.Header>() + sizeof(uint) + SharpDX.Utilities.SizeOf<DDS.HeaderDXT10>()))
                    return false;

                var headerDX10 = *(DDS.HeaderDXT10*)((byte*)headerPtr + sizeof(int) + SharpDX.Utilities.SizeOf<DDS.Header>());
        ...
        private unsafe static void EncodeDDSHeader(ImageDescription description, DDSFlags flags, IntPtr pDestination, int maxsize, out int required)
        {
            ...
            if ((flags & DDSFlags.ForceDX10Ext) == 0)
            {
                switch (description.Format)
                {
                    ...
                    case Format.R32G32B32A32_Float:
                        ddpf.Size = SharpDX.Utilities.SizeOf<DDS.PixelFormat>();
            ...
            required = sizeof(int) + SharpDX.Utilities.SizeOf<DDS.Header>();

            if (ddpf.Size == 0)
                required += SharpDX.Utilities.SizeOf<DDS.HeaderDXT10>();
            ...
            *(uint*)(pDestination) = DDS.MagicHeader;
            ...
            SharpDX.Utilities.ClearMemory((IntPtr)header, 0, SharpDX.Utilities.SizeOf<DDS.Header>());
            header->Size = SharpDX.Utilities.SizeOf<DDS.Header>();
            ...
            if (ddpf.Size == 0)
            {
                ...
                var ext = (DDS.HeaderDXT10*)((byte*)(header) + SharpDX.Utilities.SizeOf<DDS.Header>());

                SharpDX.Utilities.ClearMemory((IntPtr)ext, 0, SharpDX.Utilities.SizeOf<DDS.HeaderDXT10>());
        ...
        public unsafe static Image LoadFromDDSMemory(IntPtr pSource, int size, bool makeACopy, GCHandle? handle)
        {
            ...
            int offset = sizeof(uint) + SharpDX.Utilities.SizeOf<DDS.Header>();
            if ((convFlags & ConversionFlags.DX10) != 0)
                offset += SharpDX.Utilities.SizeOf<DDS.HeaderDXT10>();
            ...
            for (int item = 0; item < metadata.ArraySize; ++item)
            {
                ...
                for (int level = 0; level < metadata.MipLevels; ++level)
                {
                    for (int slice = 0; slice < d; ++slice)
                    {
                        ...
                        SharpDX.Utilities.Read(pixelBuffers[index].DataPointer, buffer, 0, pixsize);
            ...
            for (int arrayIndex = 0; arrayIndex < metadata.ArraySize; arrayIndex++)
            {
                ...
                for (int level = 0; level < metadata.MipLevels; ++level)
                {
                    for (int slice = 0; slice < d; ++slice, ++index)
                    {
                        ...
                        {
                            SharpDX.Utilities.CopyMemory(pDest, pSrc, Math.Min(images[index].BufferStride, imagesDst[index].BufferStride));
                        }
                        else
                        {
        ...
        private unsafe static void ExpandScanline(IntPtr pDestination, int outSize, IntPtr pSource, int inSize, Format inFormat, ScanlineFlags flags)
        {
            ...
            {
                case Format.B5G6R5_UNorm:
                    // SharpDX.DXGI.Format.B5G6R5_UNorm -> SharpDX.DXGI.Format.R8G8B8A8_UNorm
                    {
                ...
                case Format.B5G5R5A1_UNorm:
                    // SharpDX.DXGI.Format.B5G5R5A1_UNorm -> SharpDX.DXGI.Format.R8G8B8A8_UNorm
                    {
                        var sPtr = (ushort*)(pSource);
                        var dPtr = (uint*)(pDestination);
...
#if DIRECTX11_1
                case Format.B4G4R4A4_UNorm:
                    // SharpDX.DXGI.Format.B4G4R4A4_UNorm -> SharpDX.DXGI.Format.R8G8B8A8_UNorm
                    {
                        var sPtr = (ushort*)(pSource);
                        var dPtr = (uint*)(pDestination);
        ...
        internal static unsafe void CopyScanline(IntPtr pDestination, int outSize, IntPtr pSource, int inSize, Format format, ScanlineFlags flags)
        {
            if ((flags & ScanlineFlags.SetAlpha) != 0)
            {
                switch (format)
                {
                    ...
                    //-----------------------------------------------------------------------------
                    case Format.A8_UNorm:
                        SharpDX.Utilities.ClearMemory(pDestination, 0xff, outSize);
                        return;
            ...
            SharpDX.Utilities.CopyMemory(pDestination, pSource, Math.Min(outSize, inSize));
        }
