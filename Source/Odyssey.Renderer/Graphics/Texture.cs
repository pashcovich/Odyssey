...
namespace Odyssey.Graphics
{
    ...
    public abstract class Texture : GraphicsResource, IComparable<Texture>
    {
        ...
        /// <remarks>This method is only working when called from the main thread that is accessing the main <see cref="DirectXDevice" />.</remarks>
        public unsafe bool GetData(Texture stagingTexture, DataPointer toData, int arraySlice = 0, int mipSlice = 0,
            bool doNotWait = false)
        {
            ...
            if (toData.Size > mipMapSize)
                throw new ArgumentException(
                    string.Format(
                        "Size of toData ({0} bytes) is not compatible expected size ({1} bytes) : Width * Height * Depth * sizeof(PixelFormat) size in bytes",
                        toData.Size, mipMapSize));
            ...
            var box = deviceContext.MapSubresource(stagingTexture, subResourceIndex, MapMode.Read,
                doNotWait ? MapFlags.DoNotWait : MapFlags.None);
            ...
            var boxDepthStride = Description.Depth == 1 ? box.SlicePitch : textureDepthStride;
            ...
            {
                SharpDX.Utilities.CopyMemory(toData.Pointer, box.DataPointer, mipMapSize);
            }
            else
            {
                ...
                for (int j = 0; j < depth; j++)
                {
                    ...
                    {
                        // Copy a single row
                        SharpDX.Utilities.CopyMemory(new IntPtr(destPtr), new IntPtr(sourcePtr), rowStride);
        ...
        {
            SetData(Device, fromData, arraySlice, mipSlice, region);
        }
            ...
            ResourceRegion? region = null)
        {
            ...
            // Get mipmap description for the specified mipSlice
            var mipMapDesc = GetMipMapDescription(mipSlice);
            ...
            if (region.HasValue)
            {
                ...
                if (newWidth > width)
                    throw new ArgumentException(
                        string.Format("Region width [{0}] cannot be greater than mipmap width [{1}]", newWidth, width),
                        "region");
                if (newHeight > height)
                    throw new ArgumentException(
                        string.Format("Region height [{0}] cannot be greater than mipmap height [{1}]", newHeight,
                            height), "region");
                if (newDepth > depth)
                    throw new ArgumentException(
                        string.Format("Region depth [{0}] cannot be greater than mipmap depth [{1}]", newDepth, depth),
                        "region");
            ...
            Image.ComputePitch(Description.Format, width, height, out rowStride, out textureDepthStride, out width,
                out height);
            ...
            if (fromData.Size != sizeOfTextureData)
                throw new ArgumentException(
                    string.Format(
                        "Size of toData ({0} bytes) is not compatible expected size ({1} bytes) : Width * Height * Depth * sizeof(PixelFormat) size in bytes",
                        fromData.Size, sizeOfTextureData));
            ...
            if (Description.Usage == ResourceUsage.Default)
            {
                ...
                if (region.HasValue)
                {
                    ...
                    //}
                    deviceContext.UpdateSubresource(new DataBox(sourceDataPtr, rowStride, textureDepthStride), this,
                        subResourceIndex, regionValue);
                }
                else
                {
                    deviceContext.UpdateSubresource(new DataBox(fromData.Pointer, rowStride, textureDepthStride), this,
                        subResourceIndex);
                }
            }
            else
            {
                try
                {
                    var box = deviceContext.MapSubresource(this, subResourceIndex,
                        Description.Usage == ResourceUsage.Dynamic ? MapMode.WriteDiscard : MapMode.Write, MapFlags.None);
                    ...
                    if (box.RowPitch == rowStride && boxDepthStride == textureDepthStride)
                    {
                        SharpDX.Utilities.CopyMemory(box.DataPointer, fromData.Pointer, sizeOfTextureData);
                    }
                    else
                    {
                        // Otherwise, the long way by copying each scanline
        ...
        /// See unmanaged documentation for usage and restrictions.
        /// </remarks>
        public void SetData(DirectXDevice DirectXDevice, Image image)
        {
            ...
            for (int arrayIndex = 0; arrayIndex < image.Description.ArraySize; arrayIndex++)
            {
                ...
                {
                    var pixelBuffer = image.PixelBuffer[arrayIndex, mipLevel];
                    SetData(DirectXDevice, new DataPointer(pixelBuffer.DataPointer, pixelBuffer.BufferStride),
                        arrayIndex, mipLevel);
                }
            }
        }
        ...
        internal abstract TextureView GetShaderResourceView(Format viewFormat, ViewType viewType, int arrayOrDepthSlice,
            int mipIndex);
                    ...
                    if (shaderResourceViews != null)
                    {
                        ...
                        foreach (var shaderResourceViewItem in shaderResourceViews)
                        {
                            ...
                            if (shaderResourceView != null)
                                shaderResourceView.View.DebugName = Name == null
                                    ? null
                                    : String.Format("{0} SRV[{1}]", i, Name);
                            i++;
                        }
                    }
                    ...
                    if (renderTargetViews != null)
                    {
                        for (int i = 0; i < renderTargetViews.Length; i++)
                        {
                            ...
                            if (renderTargetView != null)
                                renderTargetView.View.DebugName = Name == null
                                    ? null
                                    : String.Format("{0} RTV[{1}]", i, Name);
                        }
                    }
                    ...
                    if (unorderedAccessViews != null)
                    {
                        for (int i = 0; i < unorderedAccessViews.Length; i++)
                        {
                            ...
                            if (unorderedAccessView != null)
                                unorderedAccessView.DebugName = Name == null
                                    ? null
                                    : String.Format("{0} UAV[{1}]", i, Name);
                        }
                    }
                }
            }
