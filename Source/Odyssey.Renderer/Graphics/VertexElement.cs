...
namespace Odyssey.Graphics
{
    ...
    public struct VertexElement : IEquatable<VertexElement>, IDataSerializable
    {
        ...
        /// </remarks>
        public VertexElement(string semanticName, Format format)
            : this()
        {
        ...
        public VertexElement(string semanticName, int semanticIndex, Format format, int alignedByteOffset = AppendAligned)
            : this()
        {
            if (semanticName == null)
                throw new ArgumentNullException("semanticName");
...
#endif
            {
                var attributes = SharpDX.Utilities.GetCustomAttributes<VertexElementAttribute>(field);
