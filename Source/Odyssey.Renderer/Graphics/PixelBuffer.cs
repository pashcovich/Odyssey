...
namespace Odyssey.Graphics
{
    ...
    public sealed class PixelBuffer
    {
        ...
        public PixelBuffer(int width, int height, Format format, int rowStride, int bufferStride, IntPtr dataPointer)
        {
            ...
            pixelSize = (int)FormatHelper.SizeOfInBytes(this.format);
        ...
        public unsafe void CopyTo(PixelBuffer pixelBuffer)
        {
            ...
            {
                SharpDX.Utilities.CopyMemory(pixelBuffer.DataPointer, DataPointer, BufferStride);
            }
            else
            {
                ...
                {
                    SharpDX.Utilities.CopyMemory(new IntPtr(dstPointer), new IntPtr(srcPointer), rowStride);
        ...
        {
            return SharpDX.Utilities.Read<T>(new IntPtr(((byte*)DataPointer + RowStride * y + x * PixelSize)));
        }
        ...
        {
            SharpDX.Utilities.Write(new IntPtr((byte*)DataPointer + RowStride * y + x * PixelSize), ref value);
        }
        ...
        {
            var sizeOfOutputPixel = SharpDX.Utilities.SizeOf<T>();
        ...
        public unsafe void GetPixels<T>(T[] pixels, int yOffset, int pixelIndex, int pixelCount) where T : struct
        {
            ...
            {
                SharpDX.Utilities.Read(new IntPtr(pixelPointer), pixels, 0, pixelCount);
            }
            else
            {
                var sizeOfOutputPixel = SharpDX.Utilities.SizeOf<T>() * pixelCount;
                var sizePerWidth = sizeOfOutputPixel / Width;
                var remainingPixels = sizeOfOutputPixel % Width;
                for (int i = 0; i < sizePerWidth; i++)
                {
                    SharpDX.Utilities.Read(new IntPtr(pixelPointer), pixels, pixelIndex, Width);
                    pixelPointer += rowStride;
                    pixelIndex += Width;
                }
                if (remainingPixels > 0)
                {
                    SharpDX.Utilities.Read(new IntPtr(pixelPointer), pixels, pixelIndex, remainingPixels);
                }
            }
        }
        ...
        public unsafe void SetPixels<T>(T[] sourcePixels, int yOffset, int pixelIndex, int pixelCount) where T : struct
        {
            ...
            if (isStrictRowStride)
            {
                SharpDX.Utilities.Write(new IntPtr(pixelPointer), sourcePixels, 0, pixelCount);
            }
            else
            {
                var sizeOfOutputPixel = SharpDX.Utilities.SizeOf<T>() * pixelCount;
                ...
                for (int i = 0; i < sizePerWidth; i++)
                {
                    SharpDX.Utilities.Write(new IntPtr(pixelPointer), sourcePixels, pixelIndex, Width);
                    pixelPointer += rowStride;
                    pixelIndex += Width;
                }
                if (remainingPixels > 0)
                {
                    SharpDX.Utilities.Write(new IntPtr(pixelPointer), sourcePixels, pixelIndex, remainingPixels);
                }
            }
        }
    }
