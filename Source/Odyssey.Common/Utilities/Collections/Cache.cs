        ...
        where Value : ICacheable
    {
        ...
        public int CurrentCacheUsage
        {
            get
            {
                ...
                if (cacheStore != null)
                {
                    size += cacheStore.Sum(c => (c.Value as ICacheable).BytesUsed);
                }
                return size;
            }
        }
        ...
        public int FreeSpace
        {
            get
            {
                return MaxBytes - CurrentCacheUsage;
            }
        }
        ...
        void PurgeSpace(int bytes)
        {
                ...
                cacheStore.Count != 0)
            {
                ...
                while (MaxBytes <= (CurrentCacheUsage + bytes) && arrivalOrder.Count > 0)
                {
                    ...
                    Debug.WriteLine("{0} being made available for GC freeing {1} bytes in the cache", k, freeing);
        ...
        public void Add(Key k, Value v)
        {
            ...
            {
                Debug.WriteLine("Need to make space for {0} bytes, currently using {1}", v.BytesUsed, CurrentCacheUsage);
        ...
        /// Key.
        /// </summary>
        /// <returns>The enumerator for Key.</returns>
        ...
        {
            Debug.WriteLine("Purging cached collection of {0} items", cacheStore.Count);
            arrivalOrder.Clear();
            cacheStore.Clear();
        }
    }
