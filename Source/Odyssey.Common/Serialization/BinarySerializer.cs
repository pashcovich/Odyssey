...
namespace Odyssey.Serialization
{
    ...
    ///         <li>
    ///             Supports for all primitive types, array/List&lt;T&gt;/Dictionary of primitive types, custom data via
    ///             <see cref="IDataSerializable" /> (struct or class) and array/List/Dictionary of custom data.
    ///         </li>
    ...
    public partial class BinarySerializer : Component
    {
        ...
        /// <param name="mode">The read or write mode.</param>
        /// <param name="encoding">
        ///     Default encoding used for strings. This parameter can be overriden later using Encoding
        ///     property.
        /// </param>
        ...
        /// <summary>
        ///     Register a dynamic serializer for a particular type implementing the <see cref="IDataSerializable" /> interface and
        ///     having the <see cref="DynamicSerializerAttribute" />.
        /// </summary>
...
#else
            var attribute = SharpDX.Utilities.GetCustomAttribute<DynamicSerializerAttribute>(typeof (T));
#endif
            if (attribute == null)
        ...
        /// <summary>
        ///     Register a dynamic array serializer for a particular type implementing the <see cref="IDataSerializable" />
        ///     interface.
        /// </summary>
        ...
        /// <summary>
        ///     Register a dynamic List&lt;T&gt; serializer for a particular type implementing the <see cref="IDataSerializable" />
        ///     interface.
        /// </summary>
        ...
        public void BeginChunk(FourCC chunkId)
        {
            ...
            else
            {
                ...
                var sizeOfChunk = Reader.ReadUInt32();
        ...
        /// </summary>
        /// <typeparam name="T">
        ///     Known type of the value to serialize. The known type is not the runtime type that will be actually
        ///     serialized.
        /// </typeparam>
        ...
        /// <summary>
        ///     Serializes a static value implementing the <see cref="IDataSerializable" /> interface. Unlike
        ///     <see cref="Serialize{T}(ref T)" />,
        ...
        /// <summary>
        ///     Serializes an array of primitives using serialization methods implemented by this instance for each item in the
        ///     array.
        /// </summary>
        ...
        /// <summary>
        ///     Serializes count elements in an array of primitives using serialization methods implemented by this instance for
        ...
        /// <summary>
        ///     Serializes count elements in an array of static values that are implementing the <see cref="IDataSerializable" />
        ///     interface.
        /// </summary>
        /// <typeparam name="T">Type of the data to serialize.</typeparam>
        /// <param name="valueArray">An array of value to serialize</param>
        ...
        /// <summary>
        ///     Serializes count elements from a list of static values that are implementing the <see cref="IDataSerializable" />
        ///     interface.
        /// </summary>
        /// <typeparam name="T">Type of the data to serialize.</typeparam>
        /// <param name="valueList">A list of value to serialize</param>
        ...
        public void Serialize<T>(ref List<T> valueList, int count, SerializerPrimitiveAction<T> serializerMethod,
            SerializeFlags serializeFlags = SerializeFlags.Normal)
        {
            int storeObjectRef;
            if (SerializeIsNull(ref valueList, out storeObjectRef, serializeFlags))
                return;
            ...
            where TValue : IDataSerializable, new()
        {
            ...
            else
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    var localKey = default(TKey);
                    var localValue = default(TValue);
                    localKey.Serialize(this);
                    localValue.Serialize(this);
        ...
        ///     Note that depending on the serialization <see cref="Mode" />, this method reads or writes the value.
        /// </remarks>
        public void Serialize<TKey, TValue>(ref Dictionary<TKey, TValue> dictionary, SerializerPrimitiveAction<TValue> valueSerializer,
            SerializeFlags serializeFlags = SerializeFlags.Normal) where TKey : IDataSerializable, new()
        {
            int storeObjectRef;
            if (SerializeIsNull(ref dictionary, out storeObjectRef, serializeFlags))
                return;
            ...
            else
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    var localKey = Utilities.IsValueType(typeof (TKey)) ? default(TKey) : new TKey();
                    var localValue = default(TValue);
                    localKey.Serialize(this);
                    valueSerializer(ref localValue);
                    dictionary.Add(localKey, localValue);
        ...
        ///     Type of value to serialize that is implementing the <see cref="IDataSerializable" />
        ///     interface.
        /// </typeparam>
        /// <param name="dictionary">A dictionary to serialize</param>
        ...
        ///     Note that depending on the serialization <see cref="Mode" />, this method reads or writes the value.
        /// </remarks>
        public void Serialize<TKey, TValue>(ref Dictionary<TKey, TValue> dictionary, SerializerPrimitiveAction<TKey> keySerializer,
            SerializeFlags serializeFlags = SerializeFlags.Normal) where TValue : IDataSerializable, new()
        {
            int storeObjectRef;
            if (SerializeIsNull(ref dictionary, out storeObjectRef, serializeFlags))
                return;
            ...
            else
            {
                ...
                {
                    var localKey = default(TKey);
                    var localValue = Utilities.IsValueType(typeof (TValue)) ? default(TValue) : new TValue();
                    keySerializer(ref localKey);
                    localValue.Serialize(this);
                    dictionary.Add(localKey, localValue);
                }
        ...
        /// </remarks>
        public void Serialize<TKey, TValue>(ref Dictionary<TKey, TValue> dictionary, SerializerPrimitiveAction<TKey> keySerializer,
            SerializerPrimitiveAction<TValue> valueSerializer, SerializeFlags serializeFlags = SerializeFlags.Normal)
        {
            int storeObjectRef;
            if (SerializeIsNull(ref dictionary, out storeObjectRef, serializeFlags))
                return;
        ...
        /// <summary>
        ///     Serializes a single <strong>int</strong> as a packed value (from 1 byte to 5 byte. if value &lt; 128, then 1
        ///     byte...etc.)
        /// </summary>
        /// <param name="value">The value to serialize</param>
        /// <remarks>
        ...
        ///     The pointer to an unmanaged memory region. For read operation, this pointer must be allocated
        ///     by the caller.
        /// </param>
        ...
        private bool SerializeIsNull<T>(ref T value, out int storeObjectReference, SerializeFlags flags)
        {
            ...
            var isNullValue = ReferenceEquals(value, null);
            if ((flags & SerializeFlags.Nullable) != 0 || allowIdentityReferenceCount > 0)
            {
                ...
                // Handle read
                value = default(T);
                var objectPosition = (int) Stream.Position;
        ...
        private String ReadString(bool readNullTerminatedString, int stringLength)
        {
            ...
            if (readNullTerminatedString)
            {
                ...
                var charRead = decoder.GetChars(largeByteBuffer, 0, currPos, largeCharBuffer, 0);
        ...
        private void WriteString(String value, bool writeNullTerminated, int len)
        {
            if (value == null)
                throw new ArgumentNullException("value");
        ...
        protected void WriteArrayLength(int value)
        {
            switch (ArrayLengthType)
            {
                ...
                case ArrayLengthType.Byte:
                    if (value > 255)
                        throw new NotSupportedException(string.Format("Cannot serialize array length [{0}], larger then ArrayLengthType [{1}]", value, 255));
                    Writer.Write((byte) value);
                    break;
                case ArrayLengthType.UShort:
                    if (value > 65535)
                        throw new NotSupportedException(string.Format("Cannot serialize array length [{0}], larger then ArrayLengthType [{1}]", value, 65535));
                    Writer.Write((ushort) value);
                    break;
                case ArrayLengthType.Int:
                    if (value < 0)
                        throw new NotSupportedException(string.Format("Cannot serialize array length [{0}], larger then ArrayLengthType [{1}]", value, 0x7FFFFFF));
                    Writer.Write(value);
                    break;
            }
        }
