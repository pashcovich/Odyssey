...
namespace Odyssey.Animations
{
    public class QuaternionKeyFrame : KeyFrame<Quaternion>
    {
        ...
        {
            base.OnReadXml(e);
            var value = e.XmlReader.GetAttribute("Value");

            const string pattern = @"(?<notation>[QE]+)\s*(?<values>[0-9.,\s]*)";
            var regex = new Regex(pattern);
            var matches = regex.Match(value);
            var notation = matches.Groups["notation"].Value[0];
            var values = matches.Groups["values"].Value.Split(',');

            Quaternion result;
            var qValues = (from s in values select float.Parse(s, CultureInfo.InvariantCulture)).ToArray();
            if (notation == 'E')
            {
                if (values.Length != 3)
